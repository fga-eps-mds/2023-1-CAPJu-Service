...
describe('stage endpoints', () => {
  ...
  test('new stage', async () => {
    ...
    test('new stage and delete it', async () => {
        ...
        const responseDelete = await supertest(app).delete(`/deleteStage/${response.body.idStage}`);
      ...
      test('new stage and check by id', async () => {
        ...
        const responseStage = await supertest(app).post("/newStage").send(testStage);
        expect(responseStage.status).toBe(200);


        const response = await supertest(app).get(`/stage/${responseStage.body.idStage}`);
        expect(response.status).toBe(200);
    ...
    test('new stages and list existing', async () => {
        ...
        for (const stage of testStages) {
            const testStageResponse = await supertest(app).post("/newStage").send(stage);
